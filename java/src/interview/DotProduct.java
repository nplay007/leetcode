...
public class DotProduct {
    ...
    // You have 2 sparse vectors. First tell me a way to represent and store them, and then find the dot product.
    ...
    public int SparseVectorProduct(int[] a, int[] b) {
        ...
        for (int i = 0; i < a.length; i++) {
            if (a[i] != 0) {
                l1.add(new Node(i, a[i]));
            }
        }
        for (int i = 0; i < b.length; i++) {
            if (b[i] != 0) {
                l2.add(new Node(i, b[i]));
            }
        }
        int res = 0, i = 0, j = 0;
    ...
    public int sparseVectorProduct(int[] a, int[] b) {
        ...
        for (Node n1 : l1) {
            ...
            while (left <= right) {
                ...
                if (l2.get(mid).idx == n1.idx) {
                    res += n1.val * l2.get(mid).val;
                }
                else if (l2.get(mid).idx < n1.idx) {
                    left = mid + 1;
                }
                else {
                    right = mid - 1;
