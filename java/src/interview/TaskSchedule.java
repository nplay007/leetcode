...
public class TaskSchedule {
    ...
    // 给你一个单线程的scheduler，4种thread：1，2，3，4, 冷却时间: 3，
    ...
    public int taskSchedule1(int[] tasks, int cooldown) {
        ...
        int slots = 0;
        for (int task : tasks) {
            // if we need to wait for the cool down of the same task, just update the slots
            if (map.containsKey(task) && map.get(task) > slots) {
                slots = map.get(task);
            }
            // update the time slot to the time when current task can be done again
            map.put(task, slots + 1 + cooldown);
            slots++; // update the used 1 slot of current task
        }
        return slots;
    }

    // 2. if we need to output a string of the task scheduling(without reordering), eg.1,2,1,1,3,4, k=2, -> 12_1__134
    ...
    // 3. if tasks can be reordered, output the minimum time needed: O(n) time, O(n) space
    public int taskSchedule3(int[] tasks, int cooldown) {
    ...
    /**
     * Find the task that appears for the most time Use a map to count the number of the times the task appears then get the
     * maximum count the result is decided by the maximum count and the number of tasks with maximum count
     *
     * two conditions: 1. 5 4 _ _ _ 5 4 _ _ _ 5 4 _ _ _ 5 4 the rest tasks cannot fill the empty slots 5 4 3 2 _ 5 4 3 2 _ 5 4 _ _
     * _ 5 4 the answer is (maxCount - 1) * (interval + 1) + CountOfMax 1. 5 4 _ _ _ 5 4 _ _ _ 5 4 _ _ _ 5 4 the rest tasks cannot
     * fill the empty slots 5 4 3 2 1 6 5 4 3 2 1 6 5 4 6 _ _ 5 4 the answer is the length of the nums the task which does not
     * have max count first fills the empty slots and then just insert any valid place
     */

    // output a sequence of tasks that takes least time:O(maxFrequency*klogk) time,O(n) space,n is number of unique tasks
    public String taskSchedule4(String str, int k) {
    ...
    public int taskSchedule5(int[] tasks, int cooldown) {
        if (tasks == null || tasks.length == 0) {
            return 0;
        }
